...
def _resolve_base_dir(task_id: str = "default", backend: Optional[str] = None) -> Path:
    ...
    """
    live = _get_live_tracking_cwd(task_id, backend=backend)
    if live:
...
def _resolve_path_for_task(filepath: str, task_id: str = "default",
    ...
    When *backend* is given, resolves against the cwd/home of that specific
    backend so path guards, dedup, and staleness bookkeeping are consistent
    with the environment that ``_get_file_ops(backend=...)`` will use.
    """
    raw_path = str(filepath)
    normalized_backend = _normalize_file_backend(backend)

    # For backend overrides, `~` must be interpreted in the selected backend
    # environment, not expanded with the host process home.
    ...
    if p.is_absolute():
        return p.resolve()
    return (_resolve_base_dir(task_id, backend=backend) / p).resolve()
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    try:
        ...
        with file_state.lock_path(_resolved):
            ...
            cwd_warning = _path_resolution_warning(path, Path(_resolved), task_id, backend=backend)
            file_ops = _get_file_ops(task_id, backend=backend)
        ...
        with ExitStack() as _locks:
            ...
            for _p in _paths_to_check:
                ...
                if not _sw and _r:
                    ...
                    _sw = _path_resolution_warning(_p, Path(_r), task_id, backend=backend)
                if _sw:
