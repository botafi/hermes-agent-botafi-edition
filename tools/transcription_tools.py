...
Provides speech-to-text transcription with seven providers:
  ...
  - **elevenlabs** — ElevenLabs Scribe API, requires ``ELEVENLABS_API_KEY``.
    Defaults to ``scribe_v2``; 90+ languages, word-level timestamps, optional
...
ELEVENLABS_STT_MODELS = {"scribe_v1", "scribe_v2"}
...
def _get_provider(stt_config: dict) -> str:
    ...
    # v2's accuracy and broad language coverage make it the better default than
...
def _transcribe_elevenlabs(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    may opt into ``scribe_v1`` via ``stt.elevenlabs.model``.
