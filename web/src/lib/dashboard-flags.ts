declare global {
  interface Window {
    ...
    /** Set true by the server only when HERMES_DASHBOARD_TERMINAL=1. */
    __HERMES_DASHBOARD_TERMINAL__?: boolean;
...
/** True only when the sensitive host/container Terminal tab is operator-enabled. */
export function isDashboardTerminalEnabled(): boolean {
  if (typeof window === "undefined") return false;
  return window.__HERMES_DASHBOARD_TERMINAL__ === true;
}
